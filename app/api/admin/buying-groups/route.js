...
import { writeAdminLog, undoRestoreBuyingGroup } from '@/lib/adminLog';
...
export async function GET(req) {
  ...
  const { data, error } = await admin.from('buying_groups').select('*').order('name', { ascending: true });
  if (error) {
...
export async function POST(req) {
  ...
  const admin = getSupabaseAdmin();
  try {
    const { data: prevRows } = await admin.from('buying_groups').select('*').eq('key', key).limit(1);
    const prev = prevRows?.[0] || null;

    const row = { key, name, icon_data, updated_at: new Date().toISOString() };
    const { error } = await admin.from('buying_groups').upsert(row, { onConflict: 'key' });
    if (error) throw new Error(error.message);

    await writeAdminLog(admin, me, {
      action: prev ? 'buying_group.update' : 'buying_group.create',
      target: key,
      payload: { key, name, has_icon: !!icon_data },
      undo: undoRestoreBuyingGroup(key, prev)
    });
